  ...
  for (let i = 0; i < data.results.length; ++i) {
    console.log(
      'ID-' +
        ...
        ' ' +
        '"' +
        data.results[i].speciesGuess +
        '"' +
        ' (Date: ' +
        data.results[i].created_at_details.date +
        ')'
    );
  }
}
...
function observationSummary2(data) {
  ...
  data.results.forEach((dataarray) => {
    console.log(
      'ID-' +
        dataarray.id +
        ' "' +
        dataarray.speciesGuess +
        '" (Date: ' +
        dataarray.created_at_details.date +
        ')'
    );
  });
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  const validGeoPrivacies = ['open', 'obscured', 'private', null];

  const lowerCaseGeoPrivacy = geoPrivacy !== null ? geoPrivacy.toLowerCase() : null;

  if (!validGeoPrivacies.includes(lowerCaseGeoPrivacy)) {
  ...
  const filteredObservations = data.results.filter((observation) => {
    const observationGeoPrivacy = observation.geoprivacy
      ...
      : null;
    return (
      observationGeoPrivacy === lowerCaseGeoPrivacy ||
      (observationGeoPrivacy === null && lowerCaseGeoPrivacy === null)
    );
  });
...
function transformObservation(original) {
  // TODO
  // Destructure the properties from the original object
  const { id, species_guess, quality_grade, location, photos, user } = original;

  const transformedObservation = {
    id: id,
    speciesGuess: species_guess,
    isResearchQuality: quality_grade === 'research',
    coords: location && location.includes(',') ? location.split(',').map(Number).reverse() : [],
    photos: photos ? photos.map((photo) => photo.url) : [],
    photosCount: photos ? photos.length : 0,
    user: user ? `@${user.login_exact}` : ''
  };
...
function transformObservations(data) {
  ...
  data.results.forEach((observation) => {
    var transformedObservation = transformObservation(observation);
...
function transformObservations2(data) {
  // TODO
  var transformedData = data.results.map((observation) => transformObservation(observation));
  return transformedData;
}
...
function getObservationsById(data, ...ids) {
  ...
  ids.forEach((id) => {
    var observation = data.results.find((element) => element.id == id);
    if (observation) {
...
function getPlaces(data) {
  ...
  return [places]; // Wrap the places object in an array to match the expected result
}
/*******************************************************************************
...
function getUserStats(data) {
  ...
  stats.count = data.results.length;

  data.results.forEach((observation) => {
    var user = observation.user;
...
function extractTimeZones(data) {
  ...
  data.results.forEach((observation) => {
    var createdTimeZone = observation.created_time_zone;
    var observedTimeZone = observation.observed_time_zone;

    if (createdTimeZone && !tmeZon.includes(createdTimeZone)) {
      tmeZon.push(createdTimeZone);
    }

    if (observedTimeZone && !tmeZon.includes(observedTimeZone)) {
      tmeZon.push(observedTimeZone);
    }
  });
...
function extractTimeZones2(data) {
  ...
  data.results.forEach((observation) => {
    ...
    if (createdTimeZone) {
      tmeZonSet.add(createdTimeZone);
    }

    if (observedTimeZone) {
      tmeZonSet.add(observedTimeZone);
    }
  });

  var tmeZonArray = [...tmeZonSet];
  return tmeZonArray;
}
